/**
 * soundcn - UI Sound System for Tristan Ray Portfolio
 * Components:
 *   - @soundcn/click-soft (Author: Kenney, CC0)
 *   - @soundcn/metal-click (Author: Kenney, CC0)
 * Source: https://soundcn.xyz
 */
...
(function () {
    ...
    async function playSound(soundKeyOrUri, options = {}) {
        const { volume = 0.5, playbackRate = 1 } = options;
        const ctx = getAudioContext();
        if (!ctx) return;

        let uri = soundKeyOrUri;
        if (SOUND_REGISTRY[soundKeyOrUri]) {
            uri = SOUND_REGISTRY[soundKeyOrUri].dataUri;
        }

        try {
            const buffer = await decodeDataUri(uri);
            if (!buffer) return;
    ...
    function playClickSoft(options = {}) {
        return playSound("click-soft", { volume: 0.6, ...options });
    }

    function playMetalClick(options = {}) {
        return playSound("metal-click", { volume: 0.45, ...options });
    }

    // Preload sound buffers on first user gesture
    window.addEventListener("pointerdown", () => {
        decodeDataUri(SOUND_REGISTRY["click-soft"].dataUri);
        decodeDataUri(SOUND_REGISTRY["metal-click"].dataUri);
    }, { once: true });

    // Expose soundcn API globally
    window.soundcn = {
        playClickSoft,
        playMetalClick,
        playSound,
        getAudioContext,
        registry: SOUND_REGISTRY,
        clickSoftSound: SOUND_REGISTRY["click-soft"],
        metalClickSound: SOUND_REGISTRY["metal-click"]
    };
})();
